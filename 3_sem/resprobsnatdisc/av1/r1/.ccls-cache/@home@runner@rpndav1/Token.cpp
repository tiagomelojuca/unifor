...
size_t TTokenVector::Conta(const std::vector<TToken *> &tokens,
                           ELexemes lexeme) {
  TTabelaLexemes tabelaLexemes;
...
std::vector<TToken *>
TTokenVector::TokensAEsquerda(const std::vector<TToken *> &tokens, size_t pos) {
  std::vector<TToken *> tokensEsq;

  for (int i = 0; i < pos; i++) {
    tokensEsq.push_back(tokens[i]);
  }
...
std::vector<TToken *>
TTokenVector::TokensADireita(const std::vector<TToken *> &tokens, size_t pos) {
  std::vector<TToken *> tokensDir;

  for (int i = pos + 1; i < tokens.size(); i++) {
    tokensDir.push_back(tokens[i]);
  }
