...
private:
  ...
  size_t BuscaLexemeTopLevel(const std::vector<TToken *> &tokens,
                             ELexemes lexeme) const;

  bool EhTokenTopLevel(const std::vector<TToken *> &tokens,
                       size_t posToken) const;

  EOperadorRelacional Lexeme2Operador(ELexemes lexeme) const;

  std::string Stringify(const std::vector<TToken *> &tokens) const;
